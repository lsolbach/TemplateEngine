...
//      assertEquals("AST has two children", 2, root.getChild(0).getChildNodes().size());
  ...
  public void testAstMethodDeclaration() {
    try {
      ...
      root = p.parse(tl);
      assertTrue("AST has one child", root.getChildNodes().size() == 1);
//      assertEquals("Expecting IDENTIFIER", AstNodeType.METHOD, root.getMethodNode("echo").getType());
//      assertEquals("AST has 1 child", 1, root.getChild(0).getChildNodes().size());
//      assertEquals("Expecting ARG_LIST", AstNodeType.ARG_LIST, root.getChild(0).getChild(0).getType());
    } catch (UnknownTokenException e) {
      e.printStackTrace();
    } catch (SyntaxException e) {
      e.printStackTrace();
...
//      assertEquals("Expecting ARG_LIST", AstNodeType.ARG_LIST, root.getChild(0).getChild(0).getType());
  ...
  public void testAstGenLazyEvaluation() {
    String result = "";
    st.addNewStringSymbol("a", "0");
    st.addNewStringSymbol("b", "0");
    st.addNewStringSymbol("c", "1");
    st.addNewStringSymbol("d", "1");
  ...
  public void testAstGenDecl() {
    ...
    st.addNewStringSymbol("da", "Schon da!");
    tl = t.tokenize("<?string da?><?da = 'Hello'?><?da?>");
    root = p.parse(tl);
    result = g.generate(root, st);
  ...
  public void testAstGenIf() {
    String result = "";
    ISymbolTable map = new SymbolTable();
    st.addNewStringSymbol("b", "Greetings");
    st.addNewStringSymbol("a", "Hello World");
    st.addNewMapSymbol("c", map);
    st.addNewStringSymbol("d", "b");
    st.addNewListSymbol("e", new ArrayList<ISymbol>());
  ...
  public void testAstGenDirectArrayMapAccess() {
    ...
    m = new SymbolTable();
    m.addNewStringSymbol("name", "Class2");
    classes.addNewMapSymbol(m);
    m = new SymbolTable();
    m.addNewStringSymbol("name", "Class3");
    classes.addNewMapSymbol(m);
    n = new SymbolTable();
    m.addNewMapSymbol("TaggedValueMap", n);
  ...
  public void testAstGenMethod3() {
    ...
    st.addNewStringSymbol("x", "Hallo");
    st.addNewStringSymbol("y", "Welt");
